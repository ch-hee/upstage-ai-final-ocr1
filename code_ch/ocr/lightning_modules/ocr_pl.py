...
class OCRPLModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pmap = ToPILImage(pred['prob_maps'][0])
        pmap.save(f'outputs/prob_maps/asf_True_pmap_{self.current_epoch}.png')
    ...
    def on_validation_epoch_end(self):
        ...
        # 수정 코드 
        min_coordinates = [data['min_coordinates'] for data in self.dataset['val']]
        for index, (gt_filename, gt_words) in tqdm(enumerate(self.dataset['val'].anns.items()), desc="Evaluation"):
            if gt_filename not in self.validation_step_outputs:
                # TODO: Check if this is on_sanity?
                cleval_metrics['recall'].append(np.array(0., dtype=np.float32))
                cleval_metrics['precision'].append(np.array(0., dtype=np.float32))
                ...
                continue
            #
            new_min_x, new_min_y = min_coordinates[index]
            
            pred = self.validation_step_outputs[gt_filename]
            det_quads = [[coord[i] + new_min_x if i % 2 == 0 else coord[i] + new_min_y for coord in polygons for i in range(len(coord))] for polygons in pred]
            gt_quads = [item.squeeze().reshape(-1) for item in gt_words]
    ...
    def on_test_epoch_end(self):
        ...
        # 수정 코드 
        min_coordinates = [data['min_coordinates'] for data in self.dataset['test']]
        for index, (gt_filename, gt_words) in tqdm(enumerate(self.dataset['test'].anns.items()), desc="Evaluation"):
    ...
    def on_predict_epoch_end(self):
        ...
        min_coordinates = [data['min_coordinates'] for data in self.dataset['predict']]
        for index, (filename, pred_boxes) in enumerate(self.predict_step_outputs.items()):
            new_min_x, new_min_y = min_coordinates[index]
            # Separate box
            boxes = OrderedDict()

            for idx, box in enumerate(pred_boxes):
                origin_box = [[coord + new_min_x if i == 0 else coord + new_min_y for i, coord in enumerate(coords)] for coords in box]
                boxes[f'{idx + 1:04}'] = OrderedDict(points=origin_box)
